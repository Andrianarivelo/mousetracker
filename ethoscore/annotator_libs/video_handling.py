...
class VideoPlayer(QLabel):
    ...
    label_type_changed_request = Signal(str, str, int, int) # old_behavior, new_behavior, start_frame, end_frame
    last_label_modification_requested = Signal(str, str, int, int) # action ('cut' or 'extend'), behavior, start, end
    ...
    def __init__(self, timeline=None):
        ...
        self.last_created_label = None # (behavior, start_frame, end_frame)
        ...
        self.joystick_expo = 50
        self.joystick_super_rate = 0
        self.joystick_mode = 'quadratic'
        self.frame_skip = 1
        ...
        # Deferred seek timer for smoother dragging
        self.deferred_seek_timer = QTimer()
        self.deferred_seek_timer.setSingleShot(True)
        self.deferred_seek_timer.setInterval(16)  # 60 FPS throttle for video seeking
        self.deferred_seek_timer.timeout.connect(self._perform_deferred_seek)
    ...
    def setup_ui(self):
        ...
        self.behavior_label.setStyleSheet("font-weight: bold;")
    ...
    def update_input_settings(self, settings):
        ...
        self.joystick_expo = settings.get('joystick_expo', 50) # Default 50% expo
        self.joystick_super_rate = settings.get('joystick_super_rate', 0)
    ...
    def load_video(self, video_path):
        ...
        self.progress_bar.setValue(self.current_frame + 1) # Set initial value (1-based)
    ...
    def update_frame_display(self):
        ...
        if has_actual_label:
            # Create colored parts for each behavior
            colored_parts = []
            for behavior in self.current_behavior:
                color = self.get_behavior_color(behavior)
                colored_parts.append(f'<span style="color: {color};">{behavior}</span>')
            
            label_text = ", ".join(colored_parts)
            self.behavior_label.setText(f"<b>{label_text}</b>")
            ...
            label_text = '<b><span style="color: white;">|</span></b>'
    ...
    def goto_frame(self, frame_number):
        ...
        # Update progress bar (1-based for UI and CSV)
        self.progress_bar.setValue(self.current_frame + 1)
        ...
        # Throttled update: Use deferred seek for smoother dragging
        if self.is_scrubbing:
            # If it's the first move or enough time has passed, update immediately
            # Otherwise, use the timer to throttle OpenCV seeks
            if not self.deferred_seek_timer.isActive():
                # For the very first frame of a scrub, we could update immediately,
                # but a short delay helps maintain UI fluidity if the user is just passing through.
                # However, to avoid "choppiness", we ensure it fires regularly.
                self.deferred_seek_timer.start()
            
            # Still emit frame_changed so labels and timeline update immediately
            self.frame_changed.emit(self.current_frame)
            # Redraw labels/marker without seeking the video yet
            self.update() 
        else:
            # If not scrubbing (e.g. single step), update immediately
            self.deferred_seek_timer.stop()
            self.update_frame_display()

    def _perform_deferred_seek(self):
        """Actually perform the video seek and display the frame"""
        self.update_frame_display()
    ...
    def process_gamepad_input(self):
        ...
        if self.right_stick_x_axis != -1 and self.joystick:
            ...
            if abs(axis_x_value) > self.gamepad_threshold:
                ...
                elif self.joystick_mode == 'custom':
                    # Custom expo-based scaling with super rate
                    # Rescale normalized_axis from [threshold, 1.0] to [0, 1.0] for the formula
                    x = (normalized_axis - self.gamepad_threshold) / (1.0 - self.gamepad_threshold)
                    
                    # Expo component (softens center)
                    expo = self.joystick_expo / 100.0
                    y_expo = x * (1 + expo * (x**2 - 1))
                    
                    # Super rate component (steepens end)
                    sr = min(self.joystick_super_rate / 100.0, 0.95) # Cap at 0.95 to avoid division by zero
                    speed_factor = y_expo / (1.0 - sr * x)
                else:  # linear mode
    ...
    def _process_automapped_buttons(self):
        ...
        for button_str, behavior in self.controller_mappings.items():
            ...
            if is_pressed and not self.gamepad_button_states.get(button_str, False):
                ...
                elif behavior == "move_label_up":
                    self.move_last_label(-1)
                elif behavior == "move_label_down":
                    self.move_last_label(1)
                elif behavior == "cut_label":
                    self.modify_last_label("cut")
                elif behavior == "extend_label":
                    self.modify_last_label("extend")
        ...
        if is_pressed:
            ...
            if self.label_key_mode == 'toggle':
                ...
                if not self.range_labeling_active.get(behavior, False):
                    ...
                    self.last_created_label = (behavior, start_frame, end_frame)
    ...
    def _end_timeline_drag(self, end_frame=None):
        ...
        for behavior, is_active in self.range_labeling_active.items():
            if is_active:
                ...
                self.last_created_label = (behavior, start_frame, actual_end_frame)
    ...
    def move_last_label(self, direction):
        """Move the last created label up or down in the behavior list"""
        if not self.last_created_label or not hasattr(self, 'available_behaviors'):
            return

        old_behavior, start_frame, end_frame = self.last_created_label
        if old_behavior not in self.available_behaviors:
            return

        old_idx = self.available_behaviors.index(old_behavior)
        new_idx = old_idx + direction

        if 0 <= new_idx < len(self.available_behaviors):
            new_behavior = self.available_behaviors[new_idx]
            # Emit signal to change label type
            self.label_type_changed_request.emit(old_behavior, new_behavior, start_frame, end_frame)
            # Update last created label for next move
            self.last_created_label = (new_behavior, start_frame, end_frame)

    def modify_last_label(self, action):
        """Request cut or extend on the last created label"""
        if not self.last_created_label:
            return
        
        behavior, start, end = self.last_created_label
        self.last_label_modification_requested.emit(action, behavior, start, end)
