...
from PySide6.QtGui import QPainter, QPen, QColor, QFont, QPaintEvent, QPixmap, QImage, QTransform, QPainterPath
import os
import sys

class JoystickCurveGraph(QWidget):
    """Widget to display the joystick response curve"""
    def __init__(self, parent=None):
        ...
        self.super_rate = 0.0
        self.mode = 'linear'
        self.setStyleSheet("background-color: #1a1a1a; border: 1px solid #3d3d3d; border-radius: 4px;")

    def set_params(self, deadzone, sensitivity, expo, super_rate, mode):
        self.deadzone = deadzone / 100.0
        self.sensitivity = sensitivity
        self.expo = expo / 100.0
        self.super_rate = super_rate / 100.0
        self.mode = mode
        self.update()
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(margin - 15, margin + graph_height + 15, "0")
        painter.drawText(margin + graph_width - 10, margin + graph_height + 15, "1")
        painter.drawText(margin - 15, margin + 10, "V")
        
        # Draw curve
        painter.setPen(QPen(QColor(52, 152, 219), 3))
        
        path = QPainterPath()
        path.moveTo(margin, margin + graph_height)
        
        # Max speed for normalization on Y axis
        # We'll normalize based on a reasonable max speed to keep things visible
        # A super high rate might go off-chart, we'll clip it
        chart_max_speed = 10.0 # Arbitrary max for visualization
        
        for i in range(graph_width + 1):
            x_norm = i / graph_width
            
            if x_norm < self.deadzone:
                speed_factor = 0
            else:
                if self.mode == 'linear':
                    speed_factor = x_norm
                elif self.mode == 'quadratic':
                    speed_factor = x_norm ** 2
                else: # custom
                    # Rescale x_norm from [deadzone, 1.0] to [0, 1.0]
                    x_rescaled = (x_norm - self.deadzone) / (1.0 - self.deadzone)
                    
                    # Apply Expo
                    # Formula: y = x * (1 + expo * (x^2 - 1))
                    y_expo = x_rescaled * (1 + self.expo * (x_rescaled**2 - 1))
                    
                    # Apply Super Rate (steepens the end)
                    # Use a model similar to Betaflight: factor = 1 / (1 - super_rate * x)
                    # We cap super_rate to 0.8 to avoid extreme spikes in visualization
                    sr = min(self.super_rate, 0.9)
                    speed_factor = y_expo / (1.0 - sr * x_rescaled)
            
            speed = speed_factor * (self.sensitivity / 2.0)
            # Normalize to chart height
            y_pos = margin + graph_height - (speed / chart_max_speed) * graph_height
            
            # Clip to graph area
            y_pos = max(margin, min(margin + graph_height, y_pos))
            path.lineTo(margin + i, y_pos)
...
def resource_path(relative_path):
    ...
    try:
        ...
        # Use this file's directory so assets resolve correctly regardless of cwd
...
class TimelineWidget(QWidget):
    ...
    label_type_changed = Signal(str, str, int, int)  # old_behavior, new_behavior, start_frame, end_frame
    ...
    def __init__(self):
        ...
        self.max_tracks = 1
        self._cached_sorted_behaviors = None
        self._cached_behavior_segments = None
        ...
        self.dragging_segment = None # (behavior, start_frame, end_frame)
        self.drag_preview_behavior = None
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if segment:
                ...
                self.drag_preview_behavior = segment[0]
                self.drag_current_y = event.position().y()
            ...
            self.drag_started.emit(self.current_frame)
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            if self.is_dragging_vertical:
                self.is_dragging_vertical = False
                old_behavior, start, end = self.dragging_segment
                
                # Check if it was more than a click
                if self.press_pos and (event.position() - self.press_pos).manhattanLength() >= 10:
                    if self.drag_preview_behavior and self.drag_preview_behavior != old_behavior:
                        self.label_type_changed.emit(old_behavior, self.drag_preview_behavior, start, end)
                else:
                    # It was just a click, handle as regular segment click if needed
                    segment = self.get_segment_at(event.position().x(), event.position().y())
                    if segment:
                        behavior, start, end = segment
                        clicked_frame = self.x_to_frame(event.position().x())
    ...
    def paintEvent(self, event):
        ...
        # Draw vertical drag preview if active
        self.draw_vertical_drag_preview(painter, width, height)
    ...
    def draw_vertical_drag_preview(self, painter, width, height):
        """Draw a dotted contour for vertical drag preview"""
        if not self.is_dragging_vertical or not self.dragging_segment:
            return
        ...
        # Determine current behavior track based on mouse position
        num_total_behaviors = len(sorted_behaviors)
        track_height = (height - 10) / num_total_behaviors
        
        behavior_idx = int((self.drag_current_y - 5) / track_height)
        behavior_idx = max(0, min(num_total_behaviors - 1, behavior_idx))
        target_behavior = sorted_behaviors[behavior_idx]
        self.drag_preview_behavior = target_behavior

        y_pos = 5 + behavior_idx * track_height
        
        color = self.behavior_colors.get(target_behavior, "#CCCCCC")
    ...
    def get_sorted_behaviors(self):
        """Get list of all behaviors to display as tracks with caching"""
        if not self._annotations_dirty and self._cached_sorted_behaviors is not None:
            return self._cached_sorted_behaviors

        # Use all behaviors from the behavior_colors dict to ensure fixed tracks
        # in the order they were provided (from BehaviorButtons.behaviors)
        if self.behavior_colors:
            self._cached_sorted_behaviors = list(self.behavior_colors.keys())
        else:
            # Fallback for startup if no colors provided yet
            ...
            self._cached_sorted_behaviors = sorted(list(all_present_behaviors))
    ...
    def draw_behavior_segments(self, painter, width, height):
        ...
        # Group consecutive frames for EACH behavior separately if data is dirty
        if self._annotations_dirty or self._cached_behavior_segments is None:
            self._cached_behavior_segments = {b: [] for b in sorted_behaviors} # behavior -> list of (start, end)
            
            # Efficiently group frames by behavior in one pass
            behavior_to_frames = {b: [] for b in sorted_behaviors}
            for frame, behaviors in self.annotations.items():
                if isinstance(behaviors, list):
                    for b in behaviors:
                        if b in behavior_to_frames:
                            behavior_to_frames[b].append(frame)
                elif behaviors in behavior_to_frames:
                    behavior_to_frames[behaviors].append(frame)

            for behavior in sorted_behaviors:
                # Find frames that have this behavior, sorted
                behavior_frames = sorted(behavior_to_frames[behavior])
                ...
                self._cached_behavior_segments[behavior] = segments
        ...
        for behavior_idx, behavior in enumerate(sorted_behaviors):
            segments = self._cached_behavior_segments.get(behavior, [])
            if not segments:
                continue
    ...
    def draw_range_preview(self, painter, width, height):
        ...
        for behavior, (start_frame, end_frame) in self.preview_behaviors.items():
            ...
            if behavior not in sorted_behaviors:
                continue
    ...
    def draw_frame_markers(self, painter, width, height):
        ...
        # Start from the first marker before or at scroll_offset
        start_frame_offset = int(self.scroll_offset)
        first_marker = (start_frame_offset // marker_interval) * marker_interval
        
        end_frame = min(self.total_frames, int(self.scroll_offset + width / pixels_per_frame) + 1)

        for frame in range(first_marker, end_frame, marker_interval):
            if frame < 0:
                continue
